			...
			echo'
				<div class="alcanceEquipo">
					<form class="Fila" action="Datos_Eq.php" method="post" >
						<input type="hidden" name="id" value="'.$Ligas[$i]->id.'">
						<input class="imagen" type="image" src="'.$Ligas[$i]->nombre.'" />
						<div class="datos">
							<div>Nombre: '.$Ligas[$i]->creador.'</div>
							<div>Siglas: '.$Ligas[$i]->es_publica.'</div>
							<div>Fecha de Fundacion: '.$Ligas[$i]->fecha_fundacion.'</div>
						</div>
					</form>
				</div>';
		}
